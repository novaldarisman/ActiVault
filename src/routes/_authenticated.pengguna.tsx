...
export const Route = createFileRoute("/_authenticated/pengguna")({
  ...
  beforeLoad: async () => {
    ...
    const roles: string[] = ["super_admin", "tenant_super_admin", "owner", "admin_keuangan"];
    let authorized = false;
    for (const role of roles) {
      const { data: r } = await supabase.rpc("has_role", { _user_id: u.user.id, _role: role } as any);
      if (r) { authorized = true; break; }
    }
    if (!authorized) throw redirect({ to: "/dashboard" });
  },
  head: () => ({ meta: [{ title: "Manajemen Pengguna — DocTiva" }] }),
...
function PenggunaPage() {
  ...
  const filtered: AppUser[] = (data ?? []).filter((u: any) =>
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-3xl font-semibold tracking-tight">Manajemen Pengguna</h1>
          <p className="text-muted-foreground mt-1">Kelola akun, role, dan akses sistem DocTiva</p>
        </div>
